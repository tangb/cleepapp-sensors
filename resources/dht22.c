...
static const unsigned char MAX_RETRIES = 5; // 5 * 2 = 10 seconds of max script duration
...
void toJson(float celsius, float fahrenheit, float humidity, const char* error) {
    printf("{\"celsius\": %0.2f, \"fahrenheit\": %0.2f, \"humidity\": %0.2f, \"error\": \"%s\"}\n", celsius, fahrenheit, humidity, error);
}
...
int main(int argc, char* argv[])
{
	...
	if (wiringPiSetupPhys() == -1)
	{
        ...
        toJson(0.0, 0.0, 0.0, GPIO_INIT_FAILED);
		return -126;
	}
	...
	for (unsigned char i = 0; i < MAX_RETRIES; i++)
	{
		...
		if (data[4] == checksum && checksum != 0x00)
		{
            ...
            toJson(celsius, fahrenheit, humidity, NO_ERROR);
